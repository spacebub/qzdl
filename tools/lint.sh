...
# Third party sources, the generated XPM image data and the moc output under
# the build directory are not ours to fix.
FILES=$(python3 -c '
import json, sys
...
for entry in json.load(open(db)):
    ...
    if not path.startswith(root + "/src/"):
        continue
    if "/external/" in path or "/xpm/" in path:
        continue
    print(path)
...
echo "$FILES" | xargs -P "$(nproc)" -I{} clazy-standalone \
    -p "$BUILD_DIR" \
    --ignore-dirs='.*/(external|xpm)/.*|.*_autogen/.*' \
