...
bool FilePickerDialog::closing() {
    const bool editing = State::get().filePicker.editing;
...
FilePickerDialog::FilePickerDialog(FilePicker &picker) : _picker(picker) {
    ...
    _typer = card()->append(std::make_unique<GlyphButton>(Glyphs::Glyph::Edit, [this] {
        State::FilePickerState &pick = State::get().filePicker;
    ...
    _option = card()->append(std::make_unique<Toggle>("", [this](const bool value) {
        State::get().filePicker.optionSet = value;
...
void FilePickerDialog::paintOver(const Painter &painter) {
...
Cursor FilePickerDialog::cursorAt(const double x, const double y) const {
...
std::string FilePickerDialog::said(const State::FilePickerState &pick) {
...
void FilePickerDialog::paintCrumbs(const Painter &painter) {
    const Theme::Palette &palette = Theme::of();
    const State::FilePickerState &pick = State::get().filePicker;
...
void FilePickerDialog::paintCrumb(const Painter &painter, const BLRect &box, const bool lit) {
    if (lit) {
...
int FilePickerDialog::crumbAt(const double x, const double y) const {
    if (State::get().filePicker.editing || y < _crumbs.y || y >= _crumbs.y + _crumbs.h
