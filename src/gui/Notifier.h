...
// Messages buzz into the corner of the window rather than interrupting with a box.
class Notifier {
public:
    ...
    // Errors stay until dismissed; everything else counts itself down.
    void post(Severity severity, const std::string &text, const std::string &title = {});

    void dismiss(int id);

private:
    // On screen at once before the oldest is pushed off.
