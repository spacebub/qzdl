...
void EntrySheet::opened() {
    _name->takeFocus();
}

void EntrySheet::setFile(const std::string &path) {
    _filePath = path;
...
void EntrySheet::commit() {
    if (Text::trim(_filePath).empty()) {
        return;
    }

    // Copied out: accepting takes the sheet down, and the callable with it.
