...
class EntrySheet : public toolkit::Sheet {
public:
    // `kind` is "iwad" or "port", which is what a name is guessed from.
    EntrySheet(const std::string &title, std::string kind, std::vector<std::string> filters,
               std::string remember, std::string name, std::string file,
               bool dosOffered, bool dosbox, Picker &picker,
