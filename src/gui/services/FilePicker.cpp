...
void FilePicker::upTo(const int index) {
...
bool FilePicker::hidden() {
    return Session::get().config().general.showHidden;
}

void FilePicker::showHidden(const bool value) {
    Session::get().config().general.showHidden = value;

    filePicker().hiddenShown = value;
