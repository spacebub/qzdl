...
void Engines::sweep() {
    ...
    for (size_t row = 0; row < _entries.size(); row++) {
        ...
        // Taken out first, since the handling below may start another fetch.
...
void Engines::discover() {
    ...
    for (const Detect::Found &found : Detect::ports()) {
        ...
        only = _config->lists().addPort(Convert::plain(Convert::fromPath(found.program)),
...
void Engines::remove(const int row) {
    ...
    for (int each = static_cast<int>(ListsBridge::ports().size()) - 1; each >= 0; each--) {
        if (Convert::plain(Convert::fromPath(ListsBridge::ports()[static_cast<size_t>(each)].file))
                .starts_with(root + "/")) {
