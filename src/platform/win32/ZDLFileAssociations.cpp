...
AssocListWidget::AssocListWidget(const QString &text, QListWidget *parent, const QString &prog_id, const QString &desc,
                                 const QString &exts, bool hklm, UINT icon)
        :
        QListWidgetItem(text, parent), prog_id(prog_id), desc(desc), extensions(exts.split(" ")), icon(icon),
        hklm(hklm), remove(false) {
    setFlags(Qt::ItemIsUserCheckable | Qt::ItemIsEnabled);

    bool initial = true;
    SimpleWFA::AssocStatus res_stat;
    for (const QString &ext: extensions) {
        SimpleWFA::AssocStatus cur_stat = SimpleWFA::CheckAssociationStatus(hklm, true, ext.toStdWString().c_str(),
                                                                            prog_id.toStdWString().c_str(), NULL);
        if (initial) {
            res_stat = cur_stat;
            initial = false;
...
void AssocListWidget::Process(const QString &file_path) {
    ...
    if (!remove && changed && checkState() == Qt::Checked) {
        ...
        SimpleWFA::CreateSimpleProgID(hklm, SimpleWFA::CD_UPDATE_ALWAYS, prog_id.toStdWString().c_str(),
                                      file_path.toStdWString().c_str(), desc.toStdWString().c_str(), icon);
    }
...
ZDLFileAssociations::ZDLFileAssociations(QWidget *parent) :
    ...
    assoc_list->addItem(
            new AssocListWidget("ZDL files (*.zdl)", assoc_list, "ZDL.Zdl.1", "ZDL config file", ".zdl", hklm,
                                IDI_SAVE));
    assoc_list->addItem(new AssocListWidget("WAD files (*.wad;*.iwad)", assoc_list, "ZDL.Wad.1", "Doom engine WAD file",
                                            ".wad .iwad", hklm, IDI_WAD));
    assoc_list->addItem(
            new AssocListWidget("Patch files (*.bex;*.deh)", assoc_list, "ZDL.Patch.1", "Doom engine DeHackEd patch",
                                ".bex .deh", hklm, IDI_PATCH));
    assoc_list->addItem(
            new AssocListWidget("Config files (*.cfg)", assoc_list, "ZDL.Config.1", "Doom engine config file", ".cfg",
                                hklm, IDI_CFG));
    assoc_list->addItem(
            new AssocListWidget("Demo files (*.lmp)", assoc_list, "ZDL.Demo.1", "Doom engine demo file", ".lmp", hklm,
                                IDI_DEMO));
    assoc_list->addItem(
            new AssocListWidget("Specialized archives (*.pk3;*.ipk3;*.pk7;*.ipk7;*.p7z;*.pkz;*.pke)", assoc_list,
                                "ZDL.DoomArch.1", "Doom engine specialized archive",
                                ".pk3 .ipk3 .pk7 .ipk7 .p7z .pkz .pke", hklm, IDI_ARCH));
    assoc_list->addItem(new AssocListWidget("Other supported archives (*.zip;*.7z)", assoc_list, "ZDL.OtherArch.1",
                                            "Doom engine supported archive", ".zip .7z", hklm, IDI_ARCH_RED));
