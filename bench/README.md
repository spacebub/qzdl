...
- `-DQZDL_BENCHMARK_JSON_LIBS=ON` adds `JsonLibs_*`, which measure simdjson
  (DOM and On-Demand) against yyjson over the same config files, parse only and
  through to a `Config`. It fetches simdjson. The default build measures the
  app alone.
...
| `core/`      | `Text`, `Ini`, `Json`, `Config`, `Import`, the launch layer, the WAD and PK3 readers, the JSON library comparison |
| `gui/`       | `Typeface`, `Painter`, the glyph and paint primitives, the controls, the layouts, `ReorderGrid`, `LibraryCard`, the state tree, the bridges |
...
- `Config_loadShaped` and `Config_saveShaped` sweep `Fixtures::shapes`, ports x
  profiles x add-ons per profile, from a fresh install to an 8 MB library. They
  are the startup and the write-on-edit costs of the config.
